...
function EpisodeCard({
  ...
  const triggerLabel = episode.trigger_reason ?? episode.triggered_by;
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="mb-2 flex items-center gap-2">
            ...
            />
            <div className="text-base text-[#2C2C2C]">{formatDate(episode.created_at)}</div>
          </div>
          <div className="mb-1 text-sm text-gray-500">{formatTime(episode.created_at)}</div>
          ...
          {episode.notes && !showNotes && (
            <p className="mt-2 text-sm text-gray-500 italic">&ldquo;{episode.notes}&rdquo;</p>
          )}
        </div>
      ...
      {showNotes && (
        <div className="mt-3 space-y-2">
          ...
          <div className="flex gap-2 justify-end">
            <button
              type="button"
              onClick={() => { setShowNotes(false); setNotesInput(episode.notes ?? ""); }}
...
export function HistoryScreen() {
  ...
  const episodesWithDuration = episodes.filter(e => e.duration_minutes != null);
  const avgDuration = episodesWithDuration.length > 0
    ? Math.round(episodesWithDuration.reduce((sum, ep) => sum + (ep.duration_minutes ?? 0), 0) / episodesWithDuration.length)
    : null;

  const handleShare = () => {
    const summary = `Weekly Summary\n\nTotal episodes: ${totalEpisodes}\n${avgDuration != null ? `Average duration: ${avgDuration} min\n` : ""}\nDetails:\n${episodes
      .map(ep =>
        `${formatDate(ep.created_at)} at ${formatTime(ep.created_at)} — ${ep.triggered_by}${ep.duration_minutes ? `, ${ep.duration_minutes} min` : ""}${ep.resolved ? ", completed" : ", dismissed"}`
      )
      .join("\n")}`;
    ...
    <div className="flex min-h-screen flex-col bg-[#F8F7F5] pb-24">
        ...
        <div className="mx-auto max-w-md">
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm">
            ...
            <div className="flex items-baseline gap-4">
              <div>
                <div className="text-3xl text-[#2C2C2C]">{loading ? "—" : totalEpisodes}</div>
      ...
      <div className="flex-1 px-6">
        <div className="mx-auto max-w-md space-y-3">
          {loading && (
            <div className="py-12 text-center text-gray-400">Loading episodes…</div>
          )}
          {!loading && episodes.length === 0 && (
            <div className="py-12 text-center text-gray-400">No episodes recorded yet.</div>
          )}
          {episodes.map(episode => (
            <EpisodeCard
              key={episode.id}
              episode={episode}
              onUpdateNotes={async (id, notes) => { await updateNotes(id, notes); }}
              onDelete={async (id) => { await deleteEpisode(id); }}
            />
          ))}
        </div>
      </div>
