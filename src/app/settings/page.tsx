        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <section className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:ring-offset-2 ${
                  caregiverConsent ? 'bg-zinc-900 dark:bg-zinc-100' : 'bg-zinc-200 dark:bg-zinc-700'
                }`}
              >
                <span
            ...
            {caregiverConsent && (
              <div className="space-y-4 pt-2 border-t border-zinc-100 dark:border-zinc-800">
                <label className="block">
                  <span className="text-sm text-zinc-600 dark:text-zinc-400 mb-1.5 block">Caregiver name</span>
                  <input
                    type="text"
                    value={caregiverName}
                ...
                <label className="block">
                  <span className="text-sm text-zinc-600 dark:text-zinc-400 mb-1.5 block">Caregiver email</span>
                  <input
                    type="email"
          ...
          <section className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 p-6">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(['low', 'medium', 'high'] as const).map(level => (
                <button
                  ...
                  className={`py-2 rounded-lg text-sm font-medium capitalize transition-colors focus:outline-none focus:ring-2 focus:ring-zinc-400 ${
                    sensitivity === level
            ...
            <p className="mt-3 text-xs text-zinc-400">
              Higher sensitivity triggers alerts sooner but may increase false positives.
            </p>
          </section>
